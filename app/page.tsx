...
const TASKS_KEY = "bloom:demo:tasks";
const FLOWERS_KEY = "bloom:demo:flowers";
...
export default function Home() {
  ...
  useEffect(() => {
    const storedTasks = safeParse<Task[]>(
      typeof window === "undefined" ? null : localStorage.getItem(TASKS_KEY),
      []
    );
    const storedFlowers = safeParse<Flower[]>(
      typeof window === "undefined" ? null : localStorage.getItem(FLOWERS_KEY),
      []
    );
  ...
  const todaysTasks = tasks.filter((task) => task.dateISO === todayISO);
  const todaysFlowers = flowers.filter((flower) => flower.dateISO === todayISO);
        ...
        <div className="section-card flex flex-col gap-4">
          ...
          <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(6, minmax(0, 1fr))" }}>
            {Array.from({ length: GRID_COLUMNS * GRID_ROWS }).map((_, index) => {
              ...
              const flower = todaysFlowers.find(
